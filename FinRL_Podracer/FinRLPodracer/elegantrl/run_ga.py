...
import torch
from elegantrl.env import deepcopy_or_rebuild_env
...
# todo run GA
"""GA, n = ga_num
python3 run_ga.py # ga_evaluator()
python3 demo.py   # cwd = ./demo_1
python3 demo.py   # cwd = ./demo_2
...
python3 demo.py   # cwd = ./demo_n
"""
...
def ga_evaluator():
    ...
    e_r_list = [-np.inf for _ in range(ga_num)]  # average episode return

    while not all([os.path.exists(f"{cwd_list[i]}/wait") for i in range(ga_num)]):
        time.sleep(2)
    ...
    e_r_sort = np.argsort(e_r_list)
    e_r_dst = e_r_sort[2:]
    e_r_src = e_r_dst.copy()
    e_r_src[:] = e_r_sort[0]
    e_r_src[ga_num // 2:] = e_r_sort[1]

    for src_j, dst_j in zip(e_r_src, e_r_dst):
        src_file = cwd_list[src_j]
        dst_file = cwd_list[dst_j]
        shutil.copytree(src_file, dst_file)

    # while os.path.exists(f"{cwd_list[0]}/wait"):
    #     time.sleep(2)
    for i in range(ga_num):
        cwd = cwd_list[i]
        wait_file = f'{cwd}/wait'
        shutil.rmtree(wait_file, ignore_errors=True)  # todo wait_file
...
def save_and_load_for_ga(cwd, agent, buffer):
    ...
    wait_file = f'{cwd}/wait'
    os.makedirs(wait_file, exist_ok=True)  # todo wait_file
    ...
    while if_train:
        ...
        with torch.no_grad():
            ...
            if current_epoch == target_epoch:
                current_epoch = 0
                save_and_load_for_ga(cwd, agent, buffer)
